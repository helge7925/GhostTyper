...
import { Flame, ShieldCheck, ShieldAlert, Loader2 } from 'lucide-react';
import { useTranslations } from '../../lib/i18n';
import { useUiFeedback } from '../../lib/use-ui-feedback';

const ENDPOINT = '/api/organizations/integrations/fireworks';
const TEST_ENDPOINT = '/api/organizations/integrations/fireworks/test';

export default function FireworksIntegrationPanel({ canEdit }) {
  const t = useTranslations('settings.integrations.fireworks');
  ...
  const [effectiveSource, setEffectiveSource] = useState(null);
  const [model, setModel] = useState('whisper-v3');
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setEffectiveSource(data.effectiveSource || null);
      setModel(data.effectiveModel || 'whisper-v3');
  ...
  return (
    <div className="bg-surface border border-subtle rounded-2xl p-6 shadow-xl space-y-5">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-sm font-semibold text-secondary uppercase tracking-widest flex items-center gap-2">
            <Flame className="w-4 h-4" /> {t('title')}
          </h3>
      ...
      <div className="flex items-center justify-between gap-4 bg-hover-subtle border border-subtle rounded-xl px-4 py-3">
        ...
        <label className="inline-flex items-center cursor-pointer">
          <input
            ...
            checked={enabled}
            disabled={!canEdit || saving || (!apiKeyConfigured && !apiKeyInput && !operatorFallback)}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
        <div className="bg-hover-subtle border border-subtle rounded-xl px-3 py-2">
          <dt className="text-[10px] uppercase text-secondary tracking-wider">{t('modelLabel')}</dt>
          <dd className="text-primary font-mono mt-0.5">{model}</dd>
        </div>
        <div className="bg-hover-subtle border border-subtle rounded-xl px-3 py-2">
          <dt className="text-[10px] uppercase text-secondary tracking-wider">{t('sourceLabel')}</dt>
          <dd className="text-primary mt-0.5">{sourceLabel}</dd>
        </div>
      </div>
