...
import { CHAT_TIMEOUT_MS, buildConversationMessages, buildCortecsBody, getMessageForUser, storeMessage, trimConversation, updateConversationMeta, updateUserMessageAndDeleteFollowing } from '../../../../lib/chat-service';

async function generateReply({ conversation, convId, userMessage, orgId, userId, cortecs }) {
  return withUserCostLock(userId, async () => {
    ...
    const { apiMessages, retrieval } = await buildConversationMessages({ conversation, convId, userMessage, orgId, userId, cortecs });
    ...
    const stored = await storeMessage(convId, 'assistant', result.choices?.[0]?.message?.content || '', {
      model: usedModel,
      inputTokens: usage.prompt_tokens || 0,
      outputTokens: usage.completion_tokens || 0,
      cost: estimateCost(usedModel, usage.prompt_tokens || 0, usage.completion_tokens || 0),
      metadata: { retrieval_results: retrieval.sources, edited_reply: true },
    });
...
async function handler(req, res) {
  ...
  if (!Number.isFinite(messageId) || !content) return res.status(400).json({ message: 'Nachricht und Inhalt sind erforderlich.' });
  if (content.length > 8000) return res.status(400).json({ message: 'Nachricht ist zu lang (max. 8.000 Zeichen)' });
  const allowed = await enforceRateLimit(req, res, { keyPrefix: 'chat-edit', identifier: `org:${orgId}:user:${userId}`, limit: 30, windowMs: 60_000 });
  if (!allowed) return;
  try {
    const row = await getMessageForUser(messageId, orgId, userId);
    if (!row || row.role !== 'user') return res.status(404).json({ message: 'Nachricht nicht gefunden' });
    ...
    await updateUserMessageAndDeleteFollowing({ messageId, conversationId: row.conversation_id, content });
    const reply = await generateReply({ conversation: row, convId: row.conversation_id, userMessage: content, orgId, userId, cortecs });
    await trimConversation(row.conversation_id);
    await updateConversationMeta(row.conversation_id);
    return res.status(200).json({ userMessage: { ...row, content }, message: reply });
  } catch (error) {
    ...
    logApiError('Chat edit failed', error);
    return serverError(res, 'Nachricht konnte nicht bearbeitet werden.');
  }
}
