  ...
  switch (req.method) {
    case 'PUT': {
      if (!hasPermission(req.role, 'template.write')) {
        return res.status(403).json({ code: 'FORBIDDEN', message: 'Keine Berechtigung zum Bearbeiten von Vorlagen.' });
      }
      ...
      try {
        const result = await query(
          `UPDATE templates
           ...
           RETURNING *`,
          [normalizedName, normalizedPrompt, template_type, tableSchemaForSave, categoryIdForSave, id, orgId]
        );
    ...
    case 'DELETE': {
      if (!hasPermission(req.role, 'template.delete')) {
        return res.status(403).json({ code: 'FORBIDDEN', message: 'Keine Berechtigung zum Löschen von Vorlagen.' });
      }
      try {
        const result = await query(
