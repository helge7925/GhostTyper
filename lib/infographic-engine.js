...
function parseJsonFromText(rawText) {
  ...
  function safeParse(jsonString) {
    try {
      ...
      // SECURITY: Validate parsed result is a plain object (not array, number, etc.)
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        return parsed;
      }
      return null;
    } catch {
      return undefined; // continue to next attempt
    }
  }

  // Try direct parsing first
  const directResult = safeParse(source);
  if (directResult !== undefined) return directResult;

  // Try fenced code block extraction
  const fencedMatch = source.match(/```(?:json)?\s*([\s\S]*?)```/i);
  if (fencedMatch?.[1]) {
    const fencedResult = safeParse(fencedMatch[1].trim());
    if (fencedResult !== undefined) return fencedResult;
  }

  // Try embedded JSON extraction (find first { and last })
  const start = source.indexOf('{');
  const end = source.lastIndexOf('}');
  if (start >= 0 && end > start) {
    const candidate = source.slice(start, end + 1);
    const embeddedResult = safeParse(candidate);
