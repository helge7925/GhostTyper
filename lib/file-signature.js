...
/**
 * Detects MIME types for the translate/file endpoint:
 * Office (DOCX/XLSX/PPTX) OR PDF. Returns null for anything else.
 */
export async function detectTranslatableMimeType(filePath) {
  const buffer = await readFile(filePath);
  if (startsWithBytes(buffer, Buffer.from([0x25, 0x50, 0x44, 0x46]))) {
    return 'application/pdf';
  }
  return detectOfficeMimeTypeFromBuffer(buffer);
}
