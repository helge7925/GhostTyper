 ...
 *
 * Voice handling: each request sends a `voice` preset (see the preset
 * list and per-language map below). If MISTRAL_TTS_VOICE is configured it
 * pins that voice for every request; otherwise the voice is chosen from
 * the target `language` via VOICE_BY_LANGUAGE, falling back to
...
// Transient-failure retry. Mistral occasionally answers 429 (rate limit)
  ...
  for (let attempt = 1; attempt <= TTS_MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (!isTransient || attempt === TTS_MAX_ATTEMPTS) {
        logError('tts.voxtral_failed', error, {
      ...
      logWarn('tts.voxtral_retry', { attempt, status, delayMs: delay, language });
      await new Promise((resolve) => setTimeout(resolve, delay));
    }
  }
  // Unreachable in practice (loop either returns or throws), but keep the
  // contract explicit for callers/linters.
  throw lastError || new Error('Voxtral TTS failed after retries');
}
