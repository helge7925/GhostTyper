...
// Voxtral ships 20 preset voices; `casual_male` is the one shown in the
// model's official Hugging Face code example and works against every
// language Voxtral supports (the model itself is multilingual, the
// voice just provides the timbre). Operators can override per
// deployment via MISTRAL_TTS_VOICE if they prefer e.g. casual_female,
// neutral_male, cheerful_female, etc.
 ...
 * Today: always the operator default. Wired up as a function so we can
 * add per-language overrides via env or UI without touching every call
 * site (e.g. once we know the exact preset names for the non-English
 * voices Voxtral ships).
 */
export function pickVoiceForLanguage(_language) {
