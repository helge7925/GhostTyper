...
function runCommand(cmdTemplate, filePath, timeoutMs) {
  return new Promise((resolve) => {
    ...
    // Parse command template safely - avoid shell execution
    let cmd;
    let args = [];

    if (cmdTemplate.includes('{file}')) {
      // Split by {file} placeholder and reconstruct with args
      const parts = cmdTemplate.split('{file}');
      cmd = parts[0].trim();
      // Add remaining parts as args if they exist
      if (parts.length > 1) {
        const afterFile = parts.slice(1).join('{file}').trim();
        if (afterFile) {
      ...
      // No placeholder - command at start, file path as last arg
