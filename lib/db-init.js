...
const schema = `
  ...
  CREATE TABLE IF NOT EXISTS settings (
    ...
    -- DEPRECATED: google_api_key columns are deprecated and unused (2026-04-06)
  ...
  CREATE TABLE IF NOT EXISTS realtime_sessions (
    ...
    model VARCHAR(120),
    document_template VARCHAR(120) DEFAULT 'generic',
    status VARCHAR(20) DEFAULT 'active' CHECK (status IN ('active', 'paused', 'completed')),
    transcript_text TEXT DEFAULT '',
    document_markdown TEXT DEFAULT '',
  ...
  CREATE TABLE IF NOT EXISTS realtime_session_members (
    session_id INTEGER REFERENCES realtime_sessions(id) ON DELETE CASCADE,
    user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
  ...
  CREATE TABLE IF NOT EXISTS realtime_session_events (
    id SERIAL PRIMARY KEY,
    session_id INTEGER REFERENCES realtime_sessions(id) ON DELETE CASCADE,
  ...
  CREATE TABLE IF NOT EXISTS user_workflow_versions (
    id SERIAL PRIMARY KEY,
    workflow_id INTEGER REFERENCES user_workflows(id) ON DELETE CASCADE,
...
const migrations = `
  ...
  -- DEPRECATED: google_api_key columns are deprecated and unused (2026-04-06)
  ALTER TABLE settings ADD COLUMN IF NOT EXISTS google_api_key VARCHAR(255);
  ALTER TABLE settings ADD COLUMN IF NOT EXISTS google_api_key_encrypted TEXT;
  ...
  CREATE INDEX IF NOT EXISTS idx_realtime_sessions_owner_updated ON realtime_sessions(owner_user_id, updated_at DESC);
  CREATE INDEX IF NOT EXISTS idx_realtime_sessions_status ON realtime_sessions(status);
  CREATE INDEX IF NOT EXISTS idx_realtime_members_user_session ON realtime_session_members(user_id, session_id);
  CREATE INDEX IF NOT EXISTS idx_realtime_events_session_created ON realtime_session_events(session_id, created_at DESC);
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS idx_user_workflows_user_key_unique ON user_workflows(user_id, workflow_key);
  CREATE INDEX IF NOT EXISTS idx_user_workflows_user_active ON user_workflows(user_id, is_active, updated_at DESC);
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS idx_user_workflow_versions_unique ON user_workflow_versions(workflow_id, version);
  CREATE INDEX IF NOT EXISTS idx_user_workflow_versions_workflow_created ON user_workflow_versions(workflow_id, created_at DESC);

  -- Realtime finalization state
  ALTER TABLE realtime_sessions ADD COLUMN IF NOT EXISTS document_template VARCHAR(120) DEFAULT 'generic';
  UPDATE realtime_sessions
    SET document_template = 'generic'
    WHERE document_template IS NULL OR btrim(document_template) = '';
  ALTER TABLE realtime_sessions ADD COLUMN IF NOT EXISTS finalization_state VARCHAR(20) DEFAULT 'idle';
  ALTER TABLE realtime_sessions ADD COLUMN IF NOT EXISTS finalization_error TEXT;
  ALTER TABLE realtime_sessions ADD COLUMN IF NOT EXISTS finalized_at TIMESTAMP WITH TIME ZONE;
  UPDATE realtime_sessions
    SET finalization_state = 'done'
    WHERE status = 'completed' AND (finalization_state IS NULL OR finalization_state = 'idle');
  UPDATE realtime_sessions
    SET finalization_state = 'idle'
    WHERE finalization_state IS NULL;
  CREATE INDEX IF NOT EXISTS idx_realtime_sessions_finalization ON realtime_sessions(finalization_state, updated_at DESC);
`;
