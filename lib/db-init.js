...
const migrations = `
  ...
  CREATE TABLE IF NOT EXISTS tasks (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'proposed' CHECK (status IN ('proposed', 'open', 'done', 'dismissed')),
  ...
  CREATE INDEX IF NOT EXISTS idx_tasks_org_status ON tasks(organization_id, status, updated_at DESC);
  CREATE INDEX IF NOT EXISTS idx_tasks_transcription ON tasks(transcription_id, created_at DESC);
  CREATE INDEX IF NOT EXISTS idx_tasks_assignee ON tasks(assignee_user_id, status);

  ALTER TABLE tasks ADD COLUMN IF NOT EXISTS source_segment_ids JSONB NOT NULL DEFAULT '[]'::jsonb;
  ALTER TABLE tasks ADD COLUMN IF NOT EXISTS source_chunk_id BIGINT REFERENCES document_chunks(id) ON DELETE SET NULL;
  ...
  -- Documents and knowledge bases explicitly attached to a conversation as
  -- retrieval context, in addition to the conversation's origin context_ref_id.
  CREATE TABLE IF NOT EXISTS chat_context_items (
    id BIGSERIAL PRIMARY KEY,
    conversation_id BIGINT NOT NULL REFERENCES chat_conversations(id) ON DELETE CASCADE,
    organization_id BIGINT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    context_type VARCHAR(30) NOT NULL DEFAULT 'document' CHECK (context_type IN ('document', 'knowledge_base')),
    document_id BIGINT REFERENCES documents(id) ON DELETE CASCADE,
    knowledge_base_id BIGINT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT chat_context_items_target_check CHECK (
      (context_type = 'document' AND document_id IS NOT NULL AND knowledge_base_id IS NULL)
      OR (context_type = 'knowledge_base' AND knowledge_base_id IS NOT NULL AND document_id IS NULL)
    )
  );
  CREATE INDEX IF NOT EXISTS idx_chat_context_conv ON chat_context_items(conversation_id);

  -- Workspace Wissen: shared knowledge bases grouping workspace documents for
  -- retrieval. Knowledge bases are workspace-scoped (visible to the org); only
  -- workspace-visible documents may be added (private docs stay out of shared
  -- knowledge). Directories are an optional hierarchy within a base.
  ...
  CREATE INDEX IF NOT EXISTS idx_knowledge_bases_org ON knowledge_bases(organization_id, updated_at DESC);
  ...
  CREATE TABLE IF NOT EXISTS knowledge_directories (
    ...
    organization_id BIGINT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    parent_id BIGINT REFERENCES knowledge_directories(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
  );
  CREATE INDEX IF NOT EXISTS idx_knowledge_dirs_kb ON knowledge_directories(knowledge_base_id);
  ...
  CREATE TABLE IF NOT EXISTS knowledge_items (
    ...
    organization_id BIGINT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    document_id BIGINT NOT NULL REFERENCES documents(id) ON DELETE CASCADE,
    directory_id BIGINT REFERENCES knowledge_directories(id) ON DELETE SET NULL,
    retrieval_mode VARCHAR(20) NOT NULL DEFAULT 'focused' CHECK (retrieval_mode IN ('focused', 'full_context', 'off')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(knowledge_base_id, document_id)
  );
  CREATE INDEX IF NOT EXISTS idx_knowledge_items_kb ON knowledge_items(knowledge_base_id);
  CREATE INDEX IF NOT EXISTS idx_knowledge_items_doc ON knowledge_items(document_id);

  ALTER TABLE chat_context_items ADD COLUMN IF NOT EXISTS context_type VARCHAR(30) NOT NULL DEFAULT 'document';
  ALTER TABLE chat_context_items ADD COLUMN IF NOT EXISTS knowledge_base_id BIGINT;
  ALTER TABLE chat_context_items ALTER COLUMN document_id DROP NOT NULL;
  UPDATE chat_context_items SET context_type = 'document' WHERE context_type IS NULL;
  ALTER TABLE chat_context_items DROP CONSTRAINT IF EXISTS chat_context_items_conversation_id_document_id_key;
  DO $$
  BEGIN
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'chat_context_items_knowledge_base_id_fkey'
    ) THEN
      ALTER TABLE chat_context_items ADD CONSTRAINT chat_context_items_knowledge_base_id_fkey
        FOREIGN KEY (knowledge_base_id) REFERENCES knowledge_bases(id) ON DELETE CASCADE;
    END IF;
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'chat_context_items_target_check'
    ) THEN
      ALTER TABLE chat_context_items ADD CONSTRAINT chat_context_items_target_check CHECK (
        (context_type = 'document' AND document_id IS NOT NULL AND knowledge_base_id IS NULL)
        OR (context_type = 'knowledge_base' AND knowledge_base_id IS NOT NULL AND document_id IS NULL)
      );
    END IF;
  END $$;
  CREATE UNIQUE INDEX IF NOT EXISTS idx_chat_context_doc_unique
    ON chat_context_items(conversation_id, document_id)
    WHERE document_id IS NOT NULL;
  CREATE UNIQUE INDEX IF NOT EXISTS idx_chat_context_kb_unique
    ON chat_context_items(conversation_id, knowledge_base_id)
