...
import { backfillAuditChains } from './audit-chain';
import { INITIAL_PROVIDER_PRICES, INITIAL_PRICING_EFFECTIVE_FROM } from './pricing-seed';
...
const migrations = `
  ...
  -- Vexa live STT is routed through the Cortecs Whisper bridge. Older rows
  -- were mislabeled as direct Mistral Voxtral usage; correct both attribution
  -- and the legacy estimate before taking the micro-euro backfill snapshot.
  UPDATE usage_log
     SET provider = 'cortecs',
         model = 'whisper-large-v3',
         estimated_cost_micros = ROUND(COALESCE(input_tokens, 0)::numeric * 27780000 / 1000000)::bigint,
         estimated_cost = ROUND(COALESCE(input_tokens, 0)::numeric * 27780000 / 1000000) / 1000000
   WHERE operation = 'meeting_transcription' AND model = 'voxtral-mini-latest';
  ...
  UPDATE usage_log
     SET input_quantity = COALESCE(input_quantity, input_tokens, 0),
         ...
         provider = COALESCE(provider, CASE
           WHEN model = 'whisper-v3' THEN 'fireworks'
           WHEN model LIKE 'voxtral%' OR model = 'mistral-ocr-latest' THEN 'mistral'
           ELSE 'cortecs' END),
         input_unit = COALESCE(input_unit, CASE
           WHEN model LIKE 'whisper%' OR model LIKE 'voxtral-mini-transcribe%' OR model = 'voxtral-mini-latest' THEN 'audio_second'
           WHEN model LIKE 'voxtral%tts%' THEN 'character'
           WHEN operation = 'ocr' THEN 'page'
           ELSE 'token' END),
         output_unit = COALESCE(output_unit, CASE WHEN model LIKE 'voxtral%tts%' THEN 'character' ELSE 'token' END)
  ...
  for (const row of INITIAL_PROVIDER_PRICES) {
    await client.query(
      `INSERT INTO provider_price_versions
       ...
       VALUES ($1,$2,$3,'EUR',$4,$5,$6,NULL,NULL,$7,$8,NULL)
