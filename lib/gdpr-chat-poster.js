...
const DEFAULT_NOTICE_EN = 'Notice: This meeting is being automatically transcribed for minutes. If you do not consent, please speak up now.';
 ...
 * (pages/api/webhooks/vexa.js) parallel to the share-link auto-post.
 *
...
export async function ensureGdprNoticePostedToChat({ transcriptionId, organizationId } = {}) {
  ...
  const fallbackNotice = String(vexaCfg.defaultLanguage || 'de').toLowerCase().startsWith('en')
    ? DEFAULT_NOTICE_EN
    : DEFAULT_NOTICE_DE;
