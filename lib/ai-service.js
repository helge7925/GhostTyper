...
// The Whisper transcription provider rejects audio uploads above ~25 MB. We
// keep a safety margin (multipart overhead) and, when exceeded, transcode the
// audio to a compact speech-optimised stream (mono, 16 kHz MP3) before sending.
const TRANSCRIBE_MAX_BYTES = Number.parseInt(process.env.TRANSCRIBE_MAX_BYTES, 10) || 24 * 1024 * 1024;
// When a recording is still too large after compression, split it into
// time-based chunks. We compress to ~64 kbit mono MP3 (~8 KB/s), so 20-minute
// chunks land near ~9.6 MB each — comfortably under the provider limit.
const TRANSCRIBE_CHUNK_SECONDS = Number.parseInt(process.env.TRANSCRIBE_CHUNK_SECONDS, 10) || 20 * 60;
// Consecutive chunks overlap by this many seconds so a word spoken across a
// boundary is captured in both; the duplicated text is removed when stitching.
...
// Re-encode any audio into a compact, speech-optimised MP3 (mono, 16 kHz).
// Whisper downsamples to 16 kHz internally anyway, so this is lossless w.r.t.
// transcription quality while shrinking large files by an order of magnitude.
...
// Split an (already compact) audio file into MP3 chunks of `chunkSeconds` each,
// overlapping consecutive chunks by `overlapSeconds` so a word spoken across a
// boundary is captured in both. Stream-copy keeps it fast (no re-encoding).
// Returns the chunk file paths in playback order; the i-th chunk starts at
// `i * (chunkSeconds - overlapSeconds)` seconds.
async function splitAudioIntoChunks(inputPath, chunkSeconds, overlapSeconds) {
  ...
  // We control the compressed bitrate, so estimate duration from file size
  // (avoids depending on ffprobe, which the ffmpeg installer does not ship).
  const { size } = await stat(inputPath);
  const durationSeconds = size / COMPRESSED_BYTES_PER_SECOND;
  ...
  for (let start = 0; start < durationSeconds; start += step) {
    ...
    // Sequential by design: ffmpeg seek+copy per chunk, cheap and ordered.
...
// Remove the leading words of `nextText` that duplicate the trailing words of
// `prevText` — i.e. the audible overlap between two consecutive chunks.
// Best-effort: if no confident match (>= 4 words) is found, returns nextText
// unchanged, since slight duplication is preferable to dropping words.
function stripOverlapPrefix(prevText, nextText, maxWords = 60) {
...
// Send a single audio file to the transcription endpoint and return the raw
// parsed result ({ text, segments, usage, model }). Shared by the single-file
// and chunked code paths.
async function requestTranscriptionFile(filePath, { apiKey, transcriptionModel, language, preference, contextBias, baseUrl }) {
  const audioBuffer = await readFile(filePath);
  const ext = path.extname(filePath).toLowerCase();
  const mimeType = TRANSCRIBE_MIME_BY_EXT[ext] || 'audio/mpeg';
  ...
  if (!response.ok) {
    ...
    // Cortecs nests the detail under `error.message`; fall back gracefully.
    const detail = error?.error?.message || error?.message || response.statusText;
    throw new Error(`Cortecs transcription error: ${response.status} - ${detail}`);
  }
...
export async function transcribeAudio(filePath, apiKey, options = {}) {
  ...
  // Temp files we create (conversion/compression) and must remove afterwards.
  const tempFiles = [];
  const ext = path.extname(filePath).toLowerCase();
  ...
  // The transcription provider caps uploads at ~25 MB. When the file is larger,
  // transcode it to a compact mono 16 kHz MP3 first; this fits most recordings
  // (roughly an hour) without quality loss for speech recognition.
  let needsChunking = false;
  try {
    const { size } = await stat(fileToProcess);
    if (size > TRANSCRIBE_MAX_BYTES) {
      const compressed = await compressAudioForTranscription(fileToProcess);
      tempFiles.push(compressed);
      fileToProcess = compressed;

      // Still too large even compressed (very long recording) → chunk it.
      const { size: compressedSize } = await stat(fileToProcess);
      needsChunking = compressedSize > TRANSCRIBE_MAX_BYTES;
    }
  } catch (error) {
    ...
    if (missingFfmpeg) {
      // No ffmpeg available — let the API attempt it and surface its own error.
      logWarn('audio.compression_skipped_ffmpeg_missing', { filePath });
    } else {
      // Clean up anything created before re-throwing the (already user-facing) error.
      await Promise.all(tempFiles.map((f) => unlink(f).catch(() => {})));
      throw error;
    }
  }

  const requestOptions = { apiKey, transcriptionModel, language, preference, contextBias, baseUrl };

  try {
    if (!needsChunking) {
      const result = await requestTranscriptionFile(fileToProcess, requestOptions);
    ...
    // Long recording: split into overlapping sub-limit chunks, transcribe each
    // sequentially, then stitch the texts (removing the duplicated overlap) and
    // time-shift the segments back onto one timeline.
    const chunks = await splitAudioIntoChunks(fileToProcess, TRANSCRIBE_CHUNK_SECONDS, TRANSCRIBE_CHUNK_OVERLAP_SECONDS);
    tempFiles.push(...chunks);
    if (chunks.length === 0) {
      throw new Error('Audio konnte nicht in Abschnitte zerlegt werden.');
    }
    logInfo('audio.chunked_transcription_started', { chunks: chunks.length });

    const step = TRANSCRIBE_CHUNK_SECONDS - TRANSCRIBE_CHUNK_OVERLAP_SECONDS;
    let combinedText = '';
    const mergedSegments = [];
    let totalAudioSeconds = 0;
    let usedModel = transcriptionModel;

    for (let i = 0; i < chunks.length; i += 1) {
      const chunkPath = chunks[i];
      const result = await requestTranscriptionFile(chunkPath, requestOptions);
      usedModel = result.model || usedModel;
      const timeOffset = i * step;

      const chunkText = String(result.text || '').trim();
      if (chunkText) {
        const deduped = i === 0 ? chunkText : stripOverlapPrefix(combinedText, chunkText);
        if (deduped) combinedText = combinedText ? `${combinedText} ${deduped}` : deduped;
      }

      for (const seg of (result.segments || [])) {
        // Skip segments inside the overlap already covered by the previous
        // chunk so the merged timeline has no duplicate entries.
        if (i > 0 && typeof seg.start === 'number' && seg.start < TRANSCRIBE_CHUNK_OVERLAP_SECONDS) continue;
        mergedSegments.push({
          ...seg,
          start: typeof seg.start === 'number' ? seg.start + timeOffset : seg.start,
          end: typeof seg.end === 'number' ? seg.end + timeOffset : seg.end,
        });
      }

      // Prefer the provider-reported duration; fall back to a size-based estimate
      // (compressed stream is ~64 kbit mono → COMPRESSED_BYTES_PER_SECOND). Don't
      // double-count the overlap region for billing on chunks after the first.
      const reportedSeconds = Number(result.usage?.audio_duration_seconds || 0);
      let chunkSeconds = reportedSeconds;
      if (!(chunkSeconds > 0)) {
        const { size: chunkBytes } = await stat(chunkPath);
        chunkSeconds = chunkBytes / COMPRESSED_BYTES_PER_SECOND;
      }
      totalAudioSeconds += i === 0 ? chunkSeconds : Math.max(0, chunkSeconds - TRANSCRIBE_CHUNK_OVERLAP_SECONDS);

      logInfo('audio.chunk_transcribed', { index: i + 1, of: chunks.length });
    }

    return {
      text: combinedText,
      segments: mergedSegments,
      usage: totalAudioSeconds > 0 ? { input_tokens: totalAudioSeconds, output_tokens: 0 } : {},
      model: usedModel,
    };
  } finally {
    await Promise.all(tempFiles.map((f) => unlink(f).catch(() => {})));
