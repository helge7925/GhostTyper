...
function sanitizeModelOutput(value) {
  ...
  if (typeof value === 'object') {
    const entries = Object.entries(value)
      .map(([key, nested]) => [key, sanitizeModelOutput(nested)])
...
async function convertAudioToMp3(inputPath) {
  ...
  return new Promise((resolve, reject) => {
    ffmpeg(inputPath)
      .toFormat('mp3')
      .on('error', (err) => {
        console.error('FFmpeg conversion error:', err?.message || 'unknown');
        reject(err);
      })
      .on('end', () => {
...
export async function transcribeAudio(filePath, apiKey, options = {}) {
  ...
  if (ext === '.webm' || ext === '.ogg') {
    try {
      ...
      console.error('Audio conversion failed:', error?.message || 'unknown');
...
export async function analyzeTranscription(text, template, apiKey, customPrompt = '', model = null, language = 'de') {
  ...
  try {
    ...
    if (analysis && typeof analysis === 'object') {
      analysis = sanitizeModelOutput(analysis) || {};
    }
  } catch {
...
async function deleteFileFromMistral(fileId, apiKey) {
  ...
  if (!response.ok) {
    console.warn(`Failed to delete file ${fileId} from Mistral: ${response.status}`);
  }
}
