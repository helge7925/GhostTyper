...
import { resolveVexaConfig } from './integrations.js';
import { decryptSecret, SECRET_CONTEXTS } from './secrets.js';
import { getTranscript, mapVexaTranscriptToGhostTyper, stopBot } from './api/vexa.js';
import { translateTextSegments } from './ai-service.js';
import { translateTextSegmentsEdenAi } from './edenai-service.js';
...
import { requestEmergencyBudgetStop, requestTranscriptionBudgetStop } from './budget-service.js';
import { enqueueTranslatedSegment, stopInMeetingAudio } from './in-meeting-audio.js';
import { logError, logInfo } from './observability.js';
import { splitIntoSentenceUnits, fragmentCharLength } from './sentence-buffer.js';
import {
...
async function runTranslationDelta({ slot, context, mapped }) {
  ...
  // Live-meeting STT stays on OpenRouter permanently (EdenAI's async
  // job model measured at 3.5-8.6s round-trip for a ~2s chunk against
  // Vexa's ~2-3s chunk cadence — see
  // migrate-live-meeting-stt-to-edenai/design.md's Go/No-Go
  // measurement), but live-meeting *translation* has no such latency
  // constraint (already polled independently at
  // POLL_INTERVAL_TRANSLATION_MS) and has no EdenAI-native capability
  // of its own anyway — it routes through `chat` like every other text
  // operation (see lib/edenai.js's EDENAI_HARDCODED_MODEL comment).
  ...
  const preferredModel = active.provider === 'edenai' ? active.model : active.defaultModels.chat;
