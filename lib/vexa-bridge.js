...
// fire the translation. Safety flush: if a fragment stays incomplete for
// more than this many seconds OR exceeds this many characters, translate
// it anyway so the audience doesn't fall too far behind.
 ...
 */
function splitIntoSentenceUnits(segments) {
  const units = [];
  let current = [];
  for (const seg of segments) {
    current.push(seg);
    const concatenated = current.map((s) => s.text || '').join(' ').trim();
    if (SENTENCE_TERMINATORS.test(concatenated)) {
      units.push(current);
      current = [];
    }
  }
...
function fragmentCharLength(segments) {
