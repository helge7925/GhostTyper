...
async function runTranslationDelta({ slot, context, mapped }) {
  ...
  for (const group of groups) {
    ...
    try {
      ...
      // consumer renders side-by-side). Flag the entries so the UI can
      // show them as "untranslated" and — crucially — the audio-inject
      // hook below skips them: speaking the SOURCE text in the target
      // voice would be worse than silence.
