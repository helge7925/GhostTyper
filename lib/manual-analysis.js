import { analyzeTranscription, buildTextWithSpeakers } from './ai-service.js';
import { analyzeTranscriptionEdenAi } from './edenai-service.js';
import { resolveActiveProviderConfig } from './ai-provider-router.js';
import { logApiError } from './api-utils.js';
import { query } from './db.js';
import { resolveConfiguredModel } from './openrouter.js';
import { getSettingsRow } from './settings-service.js';
...
export async function runManualAnalysisJob({ transcriptionId, userId, organizationId = null }) {
  ...
  if (!active.apiKey) {
    await markManualAnalysisError(
      transcriptionId,
      userId,
      active.provider === 'edenai' ? 'Kein EdenAI-API-Key konfiguriert' : 'Kein OpenRouter-API-Key konfiguriert',
      'Analyse ohne API-Key nicht möglich.',
      orgId
    );
    return;
  }
  // EdenAI's chat model is hardcoded (no per-user/per-request choice), so
  // this fallback-model check only applies to OpenRouter's catalogue-
  // driven selection.
  const preferredModelFallback = active.provider === 'edenai'
    ? active.model
    : resolveConfiguredModel(active, 'chat', settingsRow?.preferred_model);
  ...
  try {
    ...
    if (active.provider === 'edenai') {
      ...
      analysisModel = resolveConfiguredModel(active, 'chat', job.model) || preferredModelFallback;
    ...
    const { analysis } = await executeReservedSpend(
      {
        idempotencyKey: budgetIdempotencyKey('manual-analysis', transcriptionId),
        organizationId: orgId,
        userId,
        transcriptionId,
        operation: 'analysis',
        ...
        model: analysisModel,
        estimatedUsage: estimateTextUsage(`${analysisText}\n${job.custom_prompt || ''}`, {
