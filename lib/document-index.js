...
import { combineRetrievalScopes, partitionRetrievalModes } from './knowledge-utils';
...
async function resolveConversationRetrievalScope(conversation, organizationId, userId) {
  const ids = new Set();
  ...
  // 2) Documents and knowledge bases explicitly attached via chat_context_items.
  const convId = Number(conversation?.id);
  if (Number.isFinite(convId)) {
    const attached = await query(
      `SELECT ci.context_type, d.id AS document_id, kb.id AS knowledge_base_id
        ...
        WHERE ci.conversation_id = $1
           ...
           AND (
             (ci.context_type = 'document' AND (d.visibility = 'workspace' OR d.owner_user_id = $3))
             OR (ci.context_type = 'knowledge_base' AND kb.id IS NOT NULL)
           )`,
      [convId, organizationId, userId],
    );
    attached.rows.forEach((row) => {
      if (row.context_type === 'knowledge_base') knowledgeBaseIds.add(Number(row.knowledge_base_id));
      else ids.add(Number(row.document_id));
    });
  }

  const scopes = [{ focusedDocumentIds: Array.from(ids), fullContextDocumentIds: [] }];
  for (const knowledgeBaseId of knowledgeBaseIds) {
    if (!Number.isFinite(knowledgeBaseId)) continue;
    const items = await getKnowledgeRetrievalScope(knowledgeBaseId, organizationId, userId);
    scopes.push(partitionRetrievalModes(items));
  }

  return combineRetrievalScopes(scopes);
}

async function loadCandidateChunks(documentIds, organizationId, model, limit, message = '') {
  if (!documentIds.length) return [];
  const result = await query(
    `WITH q AS (
       SELECT NULLIF(plainto_tsquery('simple', $5)::text, '')::tsquery AS query
     )
     SELECT c.id AS chunk_id, c.document_id, c.content, c.metadata, c.chunk_index,
      ...
      ORDER BY
        CASE WHEN q.query IS NOT NULL AND c.content_tsv @@ q.query THEN ts_rank_cd(c.content_tsv, q.query) ELSE 0 END DESC,
        d.updated_at DESC,
        c.chunk_index ASC
      LIMIT $4`,
    [organizationId, documentIds, model, limit, String(message || '').slice(0, 1000)],
  );
  return result.rows;
}
...
async function rankDocumentChunks({
  ...
  const model = resolveEmbeddingModel(cortecs);
  const candidates = await loadCandidateChunks(documentIds, organizationId, model, candidateLimit, message);
...
export async function retrieveConversationSources({
  ...
  const { documentIds, fullContextDocumentIds } = await resolveConversationRetrievalScope(conversation, organizationId, userId);
  return rankDocumentChunks({ documentIds, message, organizationId, cortecs, topK, candidateLimit, contextCharLimit, fullContextDocumentIds });
}
