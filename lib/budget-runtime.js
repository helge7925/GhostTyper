...
import { resolveMistralConfig } from './settings-service.js';
...
export { composeAbortSignals, PaidJobCancelledError } from './budget-cancellation.js';
...
export async function checkpointMeetingStt({
  ...
  // Live-meeting STT is exclusively Mistral direct (see
  // lib/integrations.js's resolveBridgeTranscriptionConfig comment) —
  // this checkpoint only reserves/commits budget, it never calls the
  // provider itself (the actual transcription happens in the separate
  // Python bridge process), so it independently resolves the same
  // config to bill against the right provider/model.
  const mistral = await resolveMistralConfig({ userId, organizationId });
  const meetingModel = MISTRAL_LIVE_TRANSCRIPTION_MODEL;
  if (!mistral.apiKey) {
    throw Object.assign(new Error('Mistral live transcription is not configured.'), { code: 'MODEL_UNAVAILABLE' });
  }
