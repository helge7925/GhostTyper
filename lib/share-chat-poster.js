...
import { sendBotChatMessage } from './api/vexa.js';
import { mintShareToken, buildShareUrl, buildShareChatMessage } from './share-tokens.js';
...
export async function ensureShareLinkPostedToChat({
  ...
  try {
    ...
    // Surface in the meeting timeline so the host knows the share-link
    // never reached the chat and can paste it manually.
    await addTranscriptionEvent({
      transcriptionId,
      userId: row.user_id,
      organizationId: row.organization_id,
      stage: 'share_link_failed',
      message: `Übersetzungs-Link konnte nicht in den Meeting-Chat gepostet werden (${row.meeting_platform}). Bitte manuell teilen.`,
