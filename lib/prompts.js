...
JSON Structure:
{
  ...
  action_items: {
    de: `Du bist ein Assistent, der aus dem folgenden Text gezielt konkrete Aufgaben und To-Dos extrahiert.
Konzentriere dich ausschließlich auf umsetzbare Handlungen, Zusagen, Verantwortlichkeiten und Termine. Ignoriere allgemeine Diskussion ohne Handlungsbezug.
Schreibe knapp und präzise, ohne Floskeln oder Wiederholungen.
Wichtig: Wenn es keine Aufgaben gibt, setze "aufgaben" auf [] und erfinde keine Platzhalter.
Gib ausschließlich valides JSON aus (kein Markdown, keine Kommentare).

Struktur des JSON:
{
  "titel": "Kurzer Titel für die Aufgabenliste",
  "zusammenfassung": "Ein bis zwei Sätze Kontext, worum es geht.",
  "aufgaben": [
    {
      "aufgabe": "Konkrete, umsetzbare Beschreibung der Aufgabe",
      "verantwortlich": "Name der Person oder 'Offen'",
      "frist": "Datum oder Zeitangabe, falls genannt, sonst null",
      "prioritaet": "hoch/mittel/niedrig"
    }
  ],
  "offene_punkte": ["Unklare Punkte, die noch geklärt werden müssen"]
}`,
    en: `You are an assistant that extracts concrete action items and to-dos from the following text.
Focus exclusively on actionable steps, commitments, responsibilities, and deadlines. Ignore general discussion without an action.
Write concisely and precisely, without filler or repetition.
Important: If there are no action items, set "action_items" to [] and do not invent placeholders.
Output valid JSON only (no markdown, no comments).

JSON Structure:
{
  "title": "Short title for the action item list",
  "summary": "One or two sentences of context about what this is about.",
  "action_items": [
    {
      "task": "Concrete, actionable description of the task",
      "responsible": "Name of the person or 'Open'",
      "due": "Date or time reference if mentioned, otherwise null",
      "priority": "high/medium/low"
    }
  ],
  "open_items": ["Unclear points that still need to be clarified"]
}`
  },
  aufmass: {
...
WICHTIG: Deine eigene Antwort auf diese Aufgabe hier ist selbst reiner Fließtext, KEIN JSON und KEIN Code-Block. Die JSON-Format-Pflicht aus Punkt 2 gilt für eine ANDERE, künftige KI, die deine Anweisung später befolgt — nicht für deine jetzige Antwort. Gib also normalen, lesbaren Anweisungstext zurück, der lediglich beschreibt, welches JSON-Format diese künftige KI verwenden soll.

Gib NUR den Text der System-Anweisung zurück, ohne Einleitung, ohne Markdown-Codeblock, ohne zusätzliche Erklärungen.
