 ...
 */
export async function translateTextWithGlossaryGuard({ text, glossary = {}, translate }) {
  const relevant = selectRelevantEntries(glossary, text);
  const glossaryBlock = buildGlossaryPromptBlock(relevant);
 ...
 */
export async function translateSegmentsWithGlossaryGuard({ texts, glossary = {}, translateSegments }) {
  ...
  const glossaryBlock = buildGlossaryPromptBlock(selectRelevantEntries(glossary, safeTexts.join('\n')));
