...
import { mergeContextBias, getOrganizationContextBias } from './context-bias';
import {
  logInfo,
  trackJobCompleted,
  trackJobFailed,
...
async function processClaimedJob(job) {
  ...
  const orgBias = orgId ? await getOrganizationContextBias(orgId) : '';
  const contextBias = mergeContextBias(orgBias, settingsRow?.context_bias);
