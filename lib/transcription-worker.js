...
function mapWorkerFailure(error) {
  ...
  // Transcription/analysis errors come from Cortecs; Mistral is kept for
  // legacy rows and the OCR/TTS paths that share this pipeline.
  const provider = normalized.includes('cortecs')
    ? 'Cortecs'
...
async function processClaimedJob(job) {
  ...
  if (!cortecs.apiKey) {
    await errJob({
      message: 'Kein Cortecs API-Key konfiguriert. Bitte in den Einstellungen hinterlegen.',
  ...
  try {
    ...
    const transcriptionProvider = inferProviderForModel(cortecs.transcriptionModel);
    const transcriptionResult = await transcribeAudio(job.file_path, cortecs.apiKey, {
      contextBias,
      language,
      ...
      executeChunk: async ({ chunk, chunkIndex, execute }) => {
        ...
        return executeReservedSpend({
          ...
          model: cortecs.transcriptionModel,
          estimatedUsage: {
