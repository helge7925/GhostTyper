...
import { getSettingsRow, resolveCortecsConfig } from './settings-service';
...
import { autoIndexDocument } from './document-index';
import {
  logInfo,
  trackJobCompleted,
  trackJobFailed,
...
function mapWorkerFailure(error) {
  ...
  if (
    (normalized.includes('cortecs') || normalized.includes('mistral'))
    && (normalized.includes('401') || normalized.includes('unauthorized'))
  ) {
    return {
      message: 'Cortecs API-Key ungültig oder abgelaufen. Bitte in den Workspace-Integrationen aktualisieren.',
      eventMessage: 'Verarbeitung gestoppt: Cortecs API-Key ungültig.',
    };
  }
  ...
  if (
    ...
    return {
      message: 'Cortecs API-Limit erreicht. Bitte später erneut versuchen.',
      eventMessage: 'Verarbeitung pausiert: Cortecs API-Limit erreicht.',
    };
  }
...
async function processClaimedJob(job) {
  ...
  if (!apiKey) {
    await errJob({
      message: 'Kein Cortecs API-Key konfiguriert. Bitte in den Workspace-Integrationen hinterlegen.',
  ...
  let segments = [];
  try {
    // Budget gate first (short advisory-lock hold), then the long
    // transcription call outside the lock so it doesn't pin a pool
    // connection for the duration of the upload.
    await assertBudgetWithinLimits(userId, orgId);
    const transcriptionResult = await transcribeAudio(job.file_path, apiKey, {
      contextBias,
      language,
      transcriptionModel: cortecs.transcriptionModel,
      baseUrl: cortecs.baseUrl,
      preference: cortecs.preference,
    });
    await logUsage(userId, transcriptionResult.model, 'transcription', transcriptionResult.usage, orgId, { transcriptionId });
  ...
  if (!job.auto_analyze) {
    ...
    await autoIndexDocument({ transcriptionId, organizationId: orgId, userId, cortecs });
    return 'completed';
  }
  ...
  try {
    ...
    await assertBudgetWithinLimits(userId, orgId);
    const { analysis: analysisValue, usage: analysisUsage, model: analysisModel } = await analyzeTranscription(
      text,
      resolvedTemplate,
      apiKey,
      job.custom_prompt || '',
    ...
    await logUsage(userId, analysisModel, 'analysis', analysisUsage, orgId, { transcriptionId });
