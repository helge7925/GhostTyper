...
/**
 * Read the workspace-global context_bias for an organization. Returns the
 * raw stored string (or '' if not set). The caller is expected to pipe the
 * result through `parseContextBias` / `mergeContextBias`.
 *
 * Used by the batch transcription worker; the live/Vexa path resolves the
 * bias inside `resolveBridgeTranscriptionConfig` (single SQL hop together
 * with the API key) instead of calling this helper.
 */
export async function getOrganizationContextBias(organizationId) {
  if (!organizationId) return '';
  // Lazy import keeps the pure parser/merger functions importable from
  // unit tests that run under plain `node --test` without the bundler
  // (e.g. tests/context-bias.test.mjs).
