...
import { synthesizeSpeechEdenAi } from './edenai-service.js';
import {
  openRouterTts,
  estimatePcmDurationSeconds,
  PCM_SAMPLE_RATE,
} from './tts.js';
import { botSpeak, botSpeakStop } from './api/vexa.js';
import {
...
async function speakOne({ transcriptionId, organizationId, segment, signal: queueSignal }) {
  ...
  // Resolve TTS and Vexa delivery configuration.
  const active = await resolveActiveProviderConfig({ userId: row.user_id, organizationId, capability: 'tts' });
  if (!active.apiKey) {
    logError('in_meeting_audio.no_tts_api_key', null, { transcriptionId, provider: active.provider });
    return 0;
  }
      ...
      async (_reservation, budgetSignal) => {
        const rendered = active.provider === 'edenai'
          ? await synthesizeSpeechEdenAi({
            ...
            model: ttsModel,
            voice: ttsVoice,
            signal: composeAbortSignals(stopSignal, budgetSignal),
          })
          : await openRouterTts({
            text: segment.text,
