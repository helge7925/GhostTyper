...
import { resolveOpenRouterConfig } from './settings-service';
import {
  openRouterTts,
...
async function speakOne({ transcriptionId, organizationId, segment, signal: queueSignal }) {
  ...
  // Resolve OpenRouter TTS and Vexa delivery configuration.
  const openrouter = await resolveOpenRouterConfig({ userId: row.user_id, organizationId });
  if (!openrouter.apiKey) {
    logError('in_meeting_audio.no_openrouter_key', null, { transcriptionId });
    return 0;
  }
    ...
    const paid = await executeReservedSpend(
      {
        ...
        provider: 'openrouter',
        model: openrouter.defaultModels.tts,
      ...
      async (_reservation, budgetSignal) => {
        const rendered = await openRouterTts({
          ...
          model: openrouter.defaultModels.tts,
          voice: openrouter.ttsVoices[openrouter.defaultModels.tts],
