import { validateTableSchema } from './table-calculations.js';
import { normalizeTableSchema, orderRowsBySchema } from './table-schema.js';
...
export function normalizeAndValidateTableAnalysis(rawAnalysis, schema) {
  ...
  const schemaValidation = validateTableSchema(normalizedSchema);
  ...
  const rawMetadata = toPlainObject(analysis.metadata || analysis.metadaten);
  const metadata = {};
  ...
  for (const field of normalizedSchema.metadata) {
    const normalizedValue = normalizeByType(rawMetadata[field.key], field.type);
    metadata[field.key] = normalizedValue;
    const isMissing = normalizedValue === null || normalizedValue === '';
    if (isMissing && field.required) {
      missingMetadataFields.push(field.key);
      detailHints.push(`Metadatum fehlt: ${field.label} (${field.key})`);
    }
  }

  const rows = normalizeRows(analysis.rows);
  const orderedRows = orderRowsBySchema(rows, normalizedSchema);
  const definedRows = normalizedSchema.rows.filter((row) => row && row.key);
  ...
  for (let i = 0; i < orderedRows.length; i += 1) {
    const source = toPlainObject(orderedRows[i]);
    const target = {};
    const missing = [];
