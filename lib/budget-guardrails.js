...
/**
 * The most-restrictive of the active limits wins. Each input is in euros
 * (or null = "no limit"). The org-level limits arrive from
 * organization_settings, the personal ones from settings.
 */
export function resolveEffectiveBudgetLimit({
  costLimit = null,
  memberMonthlyBudgetLimit = null,
  organizationCostLimit = null,
  organizationMemberMonthlyBudgetLimit = null,
}) {
  const candidates = [
    toPositiveLimit(costLimit),
    toPositiveLimit(memberMonthlyBudgetLimit),
    toPositiveLimit(organizationCostLimit),
    toPositiveLimit(organizationMemberMonthlyBudgetLimit),
