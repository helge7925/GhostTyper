...
function withSettingsDefaults(settingsRow) {
  ...
  return {
    ...settingsRow,
    google_api_key: settingsRow.google_api_key ?? null,
    google_api_key_encrypted: settingsRow.google_api_key_encrypted ?? null,
...
export function resolveStoredGoogleApiKey(settingsRow) {
  if (!settingsRow) return null;

  const decrypted = decryptSecret(settingsRow.google_api_key_encrypted);
  if (decrypted) return decrypted;

  return settingsRow.google_api_key || null;
}

export function hasStoredGoogleApiKey(settingsRow) {
  if (!settingsRow) return false;
  return Boolean(settingsRow.google_api_key_encrypted || settingsRow.google_api_key);
}
