...
export function validatePriceVersion(input = {}) {
  ...
  if (input.currency && String(input.currency).toUpperCase() !== 'EUR') {
    throw Object.assign(new TypeError('Only EUR pricing is supported.'), { code: 'INVALID_PRICE' });
  }
  return {
...
export async function resolveProviderPrice({ provider, model, operation, organizationId = null, at = new Date(), client = null }) {
  ...
  const versions = await executor.query(
    `SELECT * FROM provider_price_versions
      ...
      LIMIT 2`,
    [text(provider, 'provider', 80), text(model, 'model'), text(operation, 'operation', 80), effectiveAt],
  );
