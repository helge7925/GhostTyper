...
// Pricing per 1M tokens, audio seconds, or characters (EUR/USD nominal,
// approximate). Audio rows reuse token columns: STT input audio-seconds are
// stored in `input_tokens`; Voxtral TTS synthesized text characters are stored
// in `output_tokens`. Mistral Voxtral Mini Transcribe: $0.001/min ≈
    ...
    try {
      await query(
        `INSERT INTO usage_log
           (user_id, organization_id, transcription_id, model, operation, input_tokens, output_tokens, estimated_cost)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [userId, organizationId || null, context?.transcriptionId || null, model, operation, inputTokens, outputTokens, cost]
      );
    } catch (error) {
