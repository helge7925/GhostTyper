...
function estimateWorkload({ taskType, inputText, fileSizeBytes, includePostAnalysis = false }) {
  ...
  if (taskType === 'transcription-analysis') {
    ...
    // STT pricing convention (see MODEL_PRICING in lib/usage.js): the
    // `input` rate is per 1M AUDIO SECONDS, not text tokens.
    const transcriptionCost = estimateCost('whisper-large-v3', Math.ceil(minutes * 60), 0);
