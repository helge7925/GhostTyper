...
function estimateWorkload({ taskType, inputText, fileSizeBytes, workflowSteps = 1, includePostAnalysis = false }) {
  const steps = Number.isFinite(Number(workflowSteps)) ? Math.max(1, Number(workflowSteps)) : 1;
  ...
  if (taskType === 'workflow') {
    const workflowInput = inputTokens * steps + 120 * steps;
    const workflowOutput = Math.ceil(inputTokens * 0.8 * steps + 180 * steps);
    return {
      inputTokens: workflowInput,
      outputTokens: workflowOutput,
      fixedCost: 0,
      breakdown: {
        workflow: {
          inputTokens: workflowInput,
  ...
  if (taskType === 'transcription-analysis') {
    ...
    const transcriptionCost = estimateCost('voxtral-mini-latest', 0, transcriptionOutputTokens);
  ...
  // Default: text-ai like transformations.
  return {
    inputTokens: inputTokens + 80,
