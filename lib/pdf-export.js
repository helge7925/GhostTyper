  ...
  try {
    ...
    if (error?.message?.startsWith('PDF_RENDER_FAILED:')) {
      try {
        ...
        // Fall through to sandbox fallback if allowed.
        if (disableSandbox || !isSandboxFailure(legacyError)) {
          throw legacyError;
        }
      }
    } else if (disableSandbox || !isSandboxFailure(error)) {
      throw error;
    }

    // Automatic fallback for restricted environments where Chromium sandbox cannot be initialized.
    console.warn('[PDF Export] Chromium sandbox failed, retrying with --no-sandbox');
    try {
      await runChromiumPdfOnce(inputHtmlPath, outputPdfPath, timeoutMs, {
        headlessMode: 'new',
        disableSandbox: true,
      ...
      if (noSandboxError?.message?.startsWith('PDF_RENDER_FAILED:')) {
        await runChromiumPdfOnce(inputHtmlPath, outputPdfPath, timeoutMs, {
          headlessMode: 'legacy',
          disableSandbox: true,
        });
