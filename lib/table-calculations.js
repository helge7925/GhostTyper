...
export function validateTableSchema(schema) {
  const errors = [];
  const metadata = Array.isArray(schema.metadata) ? schema.metadata : [];
  ...
  if (schema.metadata && !Array.isArray(schema.metadata)) {
    errors.push('Metadaten müssen als Array vorliegen');
  }

  if (metadata.length > 0) {
    const metadataKeys = metadata.map((field) => field.key);
    const uniqueMetadataKeys = new Set(metadataKeys);
    if (uniqueMetadataKeys.size !== metadataKeys.length) {
      errors.push('Metadaten-Keys müssen eindeutig sein');
    }

    metadata.forEach((field, idx) => {
      if (!field.key) errors.push(`Metadatum ${idx + 1}: Key fehlt`);
      if (!field.label) errors.push(`Metadatum ${idx + 1}: Bezeichnung fehlt`);
      if (!['text', 'number', 'currency', 'date'].includes(field.type)) {
        errors.push(`Metadatum ${field.key || idx + 1}: Ungültiger Typ`);
      }
    });
  }
...
export function buildTableExtractionPrompt(schema, language = 'de') {
  const metadataDefinitions = Array.isArray(schema.metadata)
    ? schema.metadata.filter((field) => field && field.key && field.label)
    : [];
  const rowDefinitions = Array.isArray(schema.rows)
    ? schema.rows.filter((row) => row && row.key && row.label)
    : [];
  const metadataBlock = metadataDefinitions.length > 0
    ? `METADATEN OBERHALB DER TABELLE:\n${metadataDefinitions.map((field) => `- "${field.key}": "${field.label}" (${field.type}${field.required ? ', erforderlich' : ''}${field.hint ? `, Hinweis: ${field.hint}` : ''})`).join('\n')}\n`
    : '';
  ...
  const rowDefinitionBlock = hasRowDefinitions
    ...
    : '';
  const jsonMetadataBlock = metadataDefinitions.length > 0
    ? `{\n${metadataDefinitions.map((field) => (
      `    "${field.key}": ${field.type === 'text' ? '"Wert"' : field.type === 'date' ? '"YYYY-MM-DD"' : '0.00'}`
    )).join(',\n')}\n  }`
    : '{}';
  const jsonRowFields = [
...
- Fülle ausschließlich Inhalte, die im Text ausdrücklich genannt oder diktiert wurden
- Berechne nichts: keine Summen, Produkte, Differenzen, Durchschnittswerte, Fortschreibungen oder Formeln
- Leite keine Werte ab, auch wenn sie naheliegend wären
- Metadaten gehören ausschließlich in "metadata" und nicht in die Tabellenzeilen
- Die Hauptdaten sind im Array "rows" (jedes Element ist eine Zeile)
- Nutze die Spaltentitel und Zeilentitel, um jeden genannten Wert in die passende Zelle einzutragen
...
- ${hasRowDefinitions ? 'Erstelle für jede vordefinierte Zeile ein Objekt und nutze zusätzlich das Feld "row_key" mit einem der vordefinierten Zeilen-Keys' : 'Optional kann ein Feld "row_key" gesetzt werden, falls Zeilenkontext vorhanden ist'}
- Bei fehlenden Werten: null oder leerer String verwenden
- Bei Zahlen: Punkt als Dezimaltrenner (z.B. 19.99)
- Bei Währungen: Nur den Zahlenwert, kein Währungszeichen
- Bei Datum: ISO-Format YYYY-MM-DD

ERFORDERLICHE JSON-STRUKTUR:
{
  "metadata": ${jsonMetadataBlock},
  "rows": [
    {
${jsonRowFields.join(',\n')}
    }
...
- Wenn Metadaten nicht genannt wurden: Metadatenwert null setzen
