 ...
 *
 * The bridge is not org-aware (Vexa-Lite does not pass an org header), so
 * exactly one workspace serves all live-transcription callers. We pick
 * the most recently updated `enabled` Mistral integration; if none exists
 * we fall back to the operator ENV key. Legacy `FIREWORKS_API_KEY` /
 * `VEXA_TRANSCRIPTION_TOKEN` values are still honoured during the
 * Voxtral migration window.
 *
  ...
  try {
    ...
    if (result.rows.length) {
      ...
      if (cfg.apiKey) {
        return {
          apiKey: cfg.apiKey,
          model: cfg.preferredModel || VOXTRAL_DEFAULT_MODEL,
