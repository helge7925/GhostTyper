...
export async function resolveBridgeTranscriptionConfig({ organizationId, platform, nativeMeetingId } = {}) {
      ...
      if (scoped.rows.length) {
        ...
        if (cfg.apiKey) {
          return {
            ...
            model: resolveTranscriptionModel(cfg.defaultTranscriptionModel, null) || CORTECS_TRANSCRIPTION_DEFAULT_MODEL,
  ...
  if (envKey) {
    return {
      ...
      model: resolveTranscriptionModel(process.env.CORTECS_TRANSCRIPTION_MODEL, null) || CORTECS_TRANSCRIPTION_DEFAULT_MODEL,
      contextBias: '',
      source: 'operator',
