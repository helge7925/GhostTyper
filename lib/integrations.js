...
import { normalizeOpenRouterConfig, OPENROUTER_BASE_URL } from './openrouter';
...
export async function resolveBridgeTranscriptionConfig({ organizationId, platform, nativeMeetingId } = {}) {
  ...
  try {
    ...
    if (scopedOrgId) {
      ...
      if (scoped.rows.length) {
        ...
        const cfg = normalizeOpenRouterConfig(parseConfig(row.config_encrypted, scopedOrgId));
        if (cfg.apiKey) {
          return {
            provider: 'openrouter',
            apiKey: cfg.apiKey,
            baseUrl: OPENROUTER_BASE_URL,
            model: cfg.defaultModels.liveTranscription || null,
            verboseJson: cfg.liveTranscriptionVerified.includes(cfg.defaultModels.liveTranscription),
  ...
  const envKey = process.env.BRIDGE_TRANSCRIPTION_API_KEY || process.env.OPENROUTER_API_KEY || null;
  if (envKey) {
    return {
      provider: 'openrouter',
      apiKey: envKey,
      baseUrl: OPENROUTER_BASE_URL,
      model: resolveTranscriptionModel(process.env.OPENROUTER_LIVE_TRANSCRIPTION_MODEL, null),
      verboseJson: false,
      contextBias: '',
      source: 'operator',
