...
async function requestBudgetStop(client, {
  ...
  const requestedByParameter = scope === 'member' ? '$4' : '$3';
  const stopped = await client.query(
    `UPDATE transcriptions
        SET cancel_requested_at = COALESCE(cancel_requested_at, NOW()),
            cancel_reason = COALESCE(cancel_reason, $2),
            cancel_requested_by = COALESCE(cancel_requested_by, ${requestedByParameter}),
      ...
      RETURNING id`,
    scope === 'member'
      ? [organizationId, reason, userId, requestedBy]
      : [organizationId, reason, requestedBy],
  );
