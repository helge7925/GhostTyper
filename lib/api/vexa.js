...
export async function startBot({ baseUrl, apiKey }, { platform, nativeMeetingId, botName, language, passcode }) {
...
const MEETING_URL_PATTERNS = [
  ...
  {
    platform: 'teams',
    test: /teams\.(?:microsoft|live)\.com\/[^\s]+meeting[^\s]+\/(19[%:][^/?#\s]+)/i,
    extract: (match) => ({ nativeMeetingId: decodeURIComponent(match[1]) }),
  },
];
