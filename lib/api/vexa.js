...
// assertOutboundUrl before creating the axios client. The baseUrl here is
// NOT org- or user-suppliable — resolveVexaConfig() (lib/integrations.js)
// hardcodes it to process.env.VEXA_BASE_URL with no org/DB override path —
// so this is the operator's own trusted, self-hosted Vexa sibling container,
// which by design always resolves to a private Docker-network address.
// We still run it through assertOutboundUrl (metadata-host/protocol checks
// stay in effect) but pass allowLoopback so the private-IP check doesn't
// block the intentional internal call.
// SSRF redirect-bypass guard: assertOutboundUrl validates only the configured
// baseUrl. axios follows redirects (default 5) without re-validating, so a
// hostile baseUrl could 30x to 127.0.0.1 or 169.254.169.254. Vexa is a
// well-known internal service that does not legitimately redirect anywhere,
// so we forbid redirects on this client entirely.
async function userClient(baseUrl, apiKey, options = {}) {
  if (!baseUrl) throw new Error('Vexa baseUrl is not configured.');
  await assertOutboundUrl(baseUrl, { allowLoopback: true });
  return axios.create({
 ...
 * loaded into that canvas via an iframe, which Teams / Zoom then
 * renders as the bot's video tile. (Not Meet — the bot no longer joins
 * Meet meetings, see the MEETING_URL_PATTERNS note below.)
 *
 * Endpoint contract:
 ...
 * generated by OpenRouter TTS in `lib/tts.js`. Keeps everything on the
 * OpenRouter key, costs are auditable per generated audio second, voice
 * stays consistent with the companion-tab.
 */
export async function botSpeak(
  { baseUrl, apiKey },
  { platform, nativeMeetingId, audioBase64, format = 'pcm', sampleRate = 24000, signal },
) {
 ...
 * (Teams chat, Zoom chat). Per-platform formatting may differ — pass
 * plain text + URL, no markdown. `platform` here is whatever the row's
 * `meeting_platform` column holds, so this function stays read/write-
 * tolerant of historic 'google_meet' rows even though new bots can no
 * longer be started against Meet.
 */
...
export async function adminHealthCheck({ baseUrl, adminToken }) {
  ...
  await assertOutboundUrl(probeUrl, { allowLoopback: true });
  try {
    // A HEAD probe checks reachability without downloading a response body.
    // Some Vexa deployments do not register HEAD for the root route and
    // answer 405; any non-5xx HTTP response still proves the service is up.
    await axios.head(probeUrl, {
      timeout: 5000,
      maxRedirects: 0,
  ...
  // Validate the admin credential separately; a public root status alone is
  // not sufficient for the meeting-start preflight.
...
// Google Meet is intentionally NOT in this list. Google hard-blocks meeting
// bots on Meet (operator-verified, July 2026: the Vexa bot lands on a
// dedicated rejection page, not a CAPTCHA) — starting a bot against a Meet
// URL is a guaranteed failure, so parseMeetingUrl() must not recognise Meet
// links as a bot-startable platform. Meet URLs are caught client-side
// (components/MeetingStartForm.js `isMeetUrl`) and redirected to tab-audio
// capture instead. Historic transcription rows with
// meeting_platform='google_meet' are unaffected — this list only feeds
// outbound bot-start requests, not reads of existing rows.
