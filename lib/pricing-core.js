...
export function calculateUsageCost(price, usage = {}) {
  ...
  const reportedCost = Number(usage.cost ?? usage.total_cost ?? usage.usage_cost);
  if (Number.isFinite(reportedCost) && reportedCost >= 0) {
    const actualCostMicros = Math.round(reportedCost * 1_000_000);
    if (!Number.isSafeInteger(actualCostMicros)) throw new RangeError('Reported cost exceeds the safe integer range.');
    return {
      ...quantities,
      standardInputQuantity: quantities.inputQuantity,
      inputCostMicros: actualCostMicros,
