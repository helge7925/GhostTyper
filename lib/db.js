...
export async function query(text, params) {
  ...
  try {
    ...
    if (duration > 100) {
      console.log('Slow query detected', { duration, rows: result.rowCount });
    }
    return result;
  } catch (error) {
    console.error('Database query error:', error?.message || 'unknown');
    throw error;
  }
}
...
export async function resolveTemplate(templateId, userId) {
  ...
  try {
    ...
    console.error('Error resolving template from DB:', error?.message || 'unknown');
  }
